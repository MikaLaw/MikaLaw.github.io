...
$(document).ready((function() {
    ...
    function animateNaturalSection(){
      ...
      $("[js-natural-section]").each((function() {
        let $section = $(this);
        let headerHeight = $(".page__header").outerHeight();
        let windowViewheight = $(window).height() - headerHeight;
        ...
        $(window).on("scroll", (function() {
          ...
          if ($section.offset().top < windowScrollTop + windowHeight ){ 
            $($section).each(function() {
              setAnimationName(this, "js-animate")
            })
          }
        })).trigger("scroll")
      }))
          ...
          limits.forEach(([leftWidth, rightWidth], i ) => {            
            if(leftWidth <= percentage && percentage <= rightWidth){
              if(i !== activeIndex) {
                ...
                setAnimationName(activeSection, 'js-animate-gorizontal')           
                $(activeSection).find('[js-animate-gorizontal]').each((i, item)=>{
                  setAnimationName(item, 'js-animate-gorizontal')
                })
              }
              activeIndex = i;
            }
    ...
    $(document).on("click", "[js-scroll][href]", (function() {
